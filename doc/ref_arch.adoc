...
NOTE:  these examples reference a DNS name of _docker_bpms_.
_docker_bpms_ is the name of the host where BPM Suite 6 is running.
Change these commands to use the IP address or DNS name of the host where your BPM Suite 6 environment is running
...
. retrieve previously inserted _policy_ facts from the working memory of the rules engine assigned to a deployment unit
+
-----
